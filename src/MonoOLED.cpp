...
#include "../include/MonoOLED.hpp"

/** @brief Wrapper for displays using the SSD 1306 driver chip. Connects to the corresponding library by Adafruit.
  *
  ...
  * Pins for talking to driver chip (SPI) must be set up in header file MonoOLED.hpp!
  * @see Display
  */

MonoOLED::MonoOLED() : hardware_(Adafruit_SSD1306(OLED_RESET))
{
    //ctor
}
...
/** @brief Switch display driver on.
  *
  */
void MonoOLED::enable()
{
    digitalWrite(ADA_SDCS, LOW);
    enabled_ = true;
}
...
/** @brief Access Adafruit library directly: Returns library object.
  *
  */
Adafruit_SSD1306 MonoOLED::accessLowlevel()
{
    return hardware_;
}

bool MonoOLED::isEnabled()
{
    return enabled_;
}
...
void MonoOLED::initDisplay()
{
    ...
    hardware_.begin(SSD1306_SWITCHCAPVCC, I2C_ADR); // Address for 128x32 is 0x3C
                                                    // Address for 128x64 is 0x3D (default) or 0x3C (if SA0 is grounded)
    hardware_.clearDisplay();
    hardware_.setRotation(2);
    enabled_ = true;
}
...
/** @brief Set basic pixel dimensions and standard rotation, define dedicated grid space and clear screen.
  *
  */
void MonoOLED::initDisplay(Shapesize gs, int gminx, int gminy, int gmaxx, int gmaxy)
{
    elemList_ = {};
    ...
    hardware_.begin(SSD1306_SWITCHCAPVCC, 0x3C);  // initialize with the I2C addr 0x3D (for the 128x64)
    hardware_.clearDisplay();
    hardware_.setRotation(2);
    enabled_ = true;
}
...
/** @brief Clear screen with dedicated color that is converted to black or white.
  *
  * Exists for compatibility reasons only.
  */
void MonoOLED::cls(Color col)
{
    hardware_.fillScreen(col.toBW());
    hardware_.display();
}

/** @brief Specialty of this driver: Show recent alterations. Must be called to see something at all.
  *
  */
void MonoOLED::refresh()
{
    hardware_.display();
    //delay(1);
}
...
/** @brief Add a high level display component to refresh list.
  ...
  * @see DisplayElement
  */
void MonoOLED::addElement(DisplayElement* de)
{
    de->setDisplay(this);
...
/** @brief Remove a high level display component from refresh list.
  ...
  * @see DisplayElement
  */
void MonoOLED::removeElement(DisplayElement* de)
{
    int ctr = 0;
...
/** @brief Retrieve component from refresh list by providing its number in the list.
  *
  */
DisplayElement* MonoOLED::getElementByNum(int num)
{
...
/** @brief Draw empty rectangle providing pixel position, size and color.
  ...
  */
void MonoOLED::drawRect(Position pixpos, Shapesize asize, Color col)
{
    hardware_.drawRect(pixpos.x, pixpos.y, asize.width, asize.height, col.toBW());
}
...
/** @brief Draw empty rectangle providing start and end pixel positions and color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawRect(Position startpos, Position endpos, Color col)
{
...
/** @brief Draw filled rectangle providing providing pixel position, size and color.
  ...
  * @see Position, Shapesize, Color
  */
void MonoOLED::fillRect(Position pixpos, Shapesize asize, Color col)
{
    hardware_.fillRect(pixpos.x, pixpos.y, asize.width, asize.height, col.toBW());
}
...
/** @brief Draw filled rectangle providing start and end pixel positions and color.
  ...
  * @see Position, Color
  */
void MonoOLED::fillRect(Position startpos, Position endpos, Color col)
{
    Shapesize asize((endpos.x - startpos.x + 1), (endpos.y - startpos.y + 1));
...
/** @brief Draw line providing start and end pixel positions and color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawLine(Position startpos, Position endpos, Color col)
{
//    if(startpos.x == endpos.x)
//    {
//        hardware_.drawFastVLine(startpos.x, startpos.y, (endpos.y - startpos.y) + 1, col.toBW());
//    }
//    else if (startpos.y == endpos.y)
//    {
//        hardware_.drawFastHLine(startpos.x, startpos.y, (endpos.x - startpos.x) + 1, col.toBW());
//    }
//    else
//    {
        hardware_.drawLine(startpos.x, startpos.y, endpos.x, endpos.y, col.toBW());
//    }
}
...
/** @brief Draw text providing pixel position, c string, foreground and background color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawText(Position pixpos, char* atext, Color fg, Color bg)
{
    hardware_.setCursor(pixpos.x, pixpos.y);
    hardware_.setTextColor(fg.toBW(), bg.toBW());
...
/** @brief Draw text providing pixel position, c string and foreground color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawText(Position pixpos, char* atext, Color fg)
{
    hardware_.setCursor(pixpos.x, pixpos.y);
    hardware_.setTextColor(fg.toBW());
    hardware_.print(atext);
}
...
/** @brief Draw text providing pixel position, c string, size, foreground and background color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawText(Position pixpos, char* atext, int asize, Color fg, Color bg)
{
...
/** @brief Draw text providing pixel position, c string, size and foreground color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawText(Position pixpos, char* atext, int asize, Color fg)
{
    hardware_.setTextSize(asize);
...
/** @brief Draw text providing grid position, c string, foreground and background color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawGridText(Position gridpos, char* atext, Color fg, Color bg)
{
...
/** @brief Draw text providing grid position, c string, size, foreground and background color.
  ...
  * @see Position, Color
  */
void MonoOLED::drawGridText(Position gridpos, char* atext, int asize, Color fg, Color bg)
{
    hardware_.setTextSize(asize);
...
/** @brief Get text box boundaries for a given string using the curretly set text size. This is just a convenience wrapper for the Adafruit GFX function with same name.
  ...
  */
void MonoOLED::getTextBounds(char* str, int16_t x, int16_t y,
...
/** @brief Set font size for next text drawing. This is just a convenience wrapper for the Adafruit GFX function with same name.
  *
  */
void MonoOLED::setFontSize(int fsize)
{
...
/** @brief Draw empty rectangle providing grid position, size and color.
  ...
  * @see Position, Shapesize, Color
  */
void MonoOLED::drawGridRect(Position gridpos, Shapesize asize, Color col)
{
    Position pixpos = gridToPix(gridpos);
...
/** @brief Draw filled rectangle providing grid position, size and color.
  ...
  * @see Position, Shapesize, Color
  */
void MonoOLED::fillGridRect(Position gridpos, Shapesize asize, Color col)
{
    Position pixpos = gridToPix(gridpos);
...
/** @brief Draw emphasized text providing grid position, c string, foreground and background color.
  ...
  * @see Position, Shapesize, Color
  */
void MonoOLED::drawGridHeadline(Position gridpos, char* atext, Color fg, Color bg)
{
    Position pixpos = gridToPix(gridpos);
    int twidth = strlen(atext) * 10;
    int theight = strlen(atext) * 14;

    hardware_.drawRect(pixpos.x, pixpos.y, twidth + 2, theight + 2, bg.toBW());
...
/** @brief Draw bitmap loaded from SD card providing a filename and grid position. Returns bitmap size.
  ...
  */
Shapesize MonoOLED::bmpGridDraw(char* filename, Position gridpos)
{
    Position pixpos = gridToPix(gridpos);
    Shapesize mysize;
...
/** @brief Retrieve size of bitmap on SD card providing a filename.
  ...
  * @see Shapesize
  */
Shapesize MonoOLED::readBmpSize(char* filename)
{
    File     bmpFile;
...
/** @brief Draw bitmap loaded from SD card providing a filename and pixel position. Returns bitmap size.
  ...
  * @see Position, Shapesize
  */
Shapesize MonoOLED::bmpDraw(char* filename, int16_t x, int16_t y)
{
    Shapesize bmpsize = Shapesize(0, 0);
    File     bmpFile;
    int      bmpWidth = 0;
...
//        Serial.println(read32(bmpFile));
        ...
        hardware_.fillRect(x, y, bmpWidth, bmpHeight, 0);
                ...
                if((x2 >= 0) && (y2 >= 0)) // On screen?
                {
                    ...
                    for (row=0; row<h; row++) // For each scanline...
                    {
                        ...
                        for (col=0; col<w; col++) // For each pixel...
                        {
                            ...
                            g = sdbuffer[buffidx++];
                            r = sdbuffer[buffidx++];
                            hardware_.drawPixel(x+col, y+row, Color(r, g, b).toBW());
